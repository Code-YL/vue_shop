...
<script>
import {getCateList,getParamsData,addParams,showEditDialog,editParams,removeParams,saveAttrVals} from 'network/goods/params'
...
export default {
  ...
  methods: {
    //   获取所有的商品分类列表
    async getCateList () {
      const res = await getCateList('categories')
    ...
    async getParamsData () {
      ...
      const res = await getParamsData(`categories/${this.getCateId}/attributes`,{ sel: this.activeTabsName })
      if (res.meta.status !== 200) {
    ...
    async showEditDialog (attrId) {
      const { data: res } = await showEditDialog(`categories/${this.getCateId}/attributes/${attrId}`,{ attr_sel: this.activeTabsName })
      if (res.meta.status !== 200) {
