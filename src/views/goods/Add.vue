...
import {getCateList,tabClicked,addGoods} from 'network/goods/add'
  ...
  methods: {
    ...
    async tabClicked () {
      ...
      if (this.activeIndex === '1') {
        const res = await tabClicked(
          `categories/${this.getCateId}/attributes`,{ sel: 'many' }
        )
        if (res.meta.status !== 200) {
        ...
        const res = await tabClicked(
          `categories/${this.getCateId}/attributes`,{ sel: 'only' }
        )
        if (res.meta.status !== 200) {
          return this.$message.error('获取动态参数列表失败！')
        }
